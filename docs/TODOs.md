...
-   [x] **핵심 목표**: 실제 운영 환경에 앱을 배포하고, API 키 유출을 원천적으로 차단하여 **서비스 수준의 보안**을 확보.
    ...
    -   [ ] **2단계 (준비)**: 배포에 필요한 기반 환경을 구축.
        -   [x] **배포 전 최종 정리**: 프로젝트에 남아있던 불필요한 `index.tsx` 파일을 완전히 제거하여 배포 안정성을 확보.
        -   [ ] **GitHub 저장소 생성 및 코드 업로드**: 코드 관리를 위한 원격 저장소(Repository)에 현재까지 작업한 모든 파일을 업로드.
        -   [ ] **Netlify 계정 연동**: Netlify에 가입하고, 생성한 GitHub 저장소를 연결하여 자동 배포 환경을 설정.

    -   [ ] **3단계 (실행)**: Netlify를 통해 앱을 배포.
        -   [ ] **환경 변수 설정**: Netlify 배포 설정에서 Gemini API 키를 `API_KEY`라는 이름의 환경 변수로 안전하게 설정. (가장 중요한 단계)
        -   [ ] **최초 배포 실행**: 설정이 완료되면, Netlify가 자동으로 첫 배포를 시작하도록 트리거.
        -   [ ] **공개 주소(URL) 확인**: 배포가 완료된 후, Netlify가 제공하는 고유한 웹 주소를 확인.

    -   [ ] **4단계 (검증 및 오류 수정)**: 배포된 앱의 모든 기능을 실제 환경에서 테스트.
        -   [ ] **핵심 기능 테스트**: 공개 주소로 접속하여 카메라, 마이크, 앨범 업로드, 해설 생성, 음성 출력, 보관함 저장/삭제 등 모든 기능이 정상 작동하는지 검증.
        -   [ ] **PWA 기능 테스트**: 스마트폰에서 '홈 화면에 추가' 기능이 작동하는지, 아이콘이 정상적으로 생성되는지 확인.
        -   [ ] **오류 수정 프로세스**: 오류 발생 시, `로컬 수정 → GitHub 업로드 → Netlify 자동 재배포` 프로세스를 통해 신속하게 문제를 해결.